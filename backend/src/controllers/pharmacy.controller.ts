...
export const getPharmacyDashboardStats = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if (role === 'PHARMACIST' || role === 'SUPER_ADMIN') {
            const [pendingRes, approvedTodayRes, clarifyRes, urgentPrescriptionsRes] = await Promise.all([
                query("SELECT COUNT(*) as count FROM prescriptions WHERE status IN ('UPLOADED', 'UNDER_REVIEW')"),
                query("SELECT COUNT(*) as count FROM prescriptions WHERE status = 'APPROVED' AND DATE(updated_at) = CURRENT_DATE"),
                query("SELECT COUNT(*) as count FROM prescriptions WHERE status = 'CLARIFICATION_REQUIRED'"),
                query(`
        ...
        if (role === 'PHARMACY_ADMIN' || role === 'SUPER_ADMIN') {
            const [newOrdersRes, processingRes, completedRes, cancelledRes, revenueRes, recentOrdersRes] = await Promise.all([
                query("SELECT COUNT(*) as count FROM orders WHERE status IN ('PENDING', 'PRESCRIPTION_PENDING')"),
                query("SELECT COUNT(*) as count FROM orders WHERE status IN ('READY_TO_SHIP', 'SHIPPED')"),
                query("SELECT COUNT(*) as count FROM orders WHERE status = 'DELIVERED'"),
                query("SELECT COUNT(*) as count FROM orders WHERE status IN ('CANCELLED', 'REJECTED')"),
